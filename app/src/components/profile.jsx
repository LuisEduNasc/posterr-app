...
export function Profile({ myUser }) {
  ...
  const {
    ...
    queryFn: async () => {
      const postsData = await fetch(
        `http://localhost:3333/posts?user_id=${userId}`
      );
  ...
  const { mutateAsync } = useMutation({
    mutationFn: async ({ follow }) => {
      const newMe = {
        following: follow
          ? user.me.following.filter((id) => id !== user.userProfile.id)
          : [...user.me.following, user.userProfile.id],
        following_count: follow
          ? user.me.following_count - 1
          : user.me.following_count + 1,
      };

      const newUserProfile = {
        followers: follow
          ? user.userProfile.followers.filter((id) => id !== user.me.id)
          : [...user.userProfile.followers, user.me.id],
    ...
    >
      ...
      >
        {isErrorUser || isLoadingUser ? (
          <DefaultMessage
            message={
              isErrorUser ? 'Something went wrong, Try again' : 'Loading...'
            }
          />
        ) : (
          <div>
